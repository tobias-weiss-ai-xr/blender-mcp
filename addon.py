...
class BlenderMCPServer:
    ...
    def _execute_command_internal(self, command):
        ...
        handlers = {
            ...
            "get_fcurve_data": self.get_fcurve_data,
            "get_nla_tracks": self.get_nla_tracks,
            "create_nla_track": self.create_nla_track,
            # Visual verification tool handlers
            "capture_multi_view_screenshots": self.capture_multi_view_screenshots,
            "export_screenshot_to_file": self.export_screenshot_to_file,
            "get_scene_analysis": self.get_scene_analysis,
    ...
    def create_material(self, name):
        ...
        try:
            ...
            mat.use_nodes = True  # Enable node tree for shader editing
    ...
    def get_node_tree(self, material_name):
        """Get the node tree structure of a material"""
        try:
    ...
    def create_shader_node(self, material_name, node_type, location=None, name=None):
    ...
    def link_shader_nodes(
        ...
        try:
            ...
            if not mat.use_nodes or not mat.node_tree:
                return {"error": "Material does not have a node tree"}

            node_tree = mat.node_tree

            # Find the nodes
            from_n = node_tree.nodes.get(from_node)
            to_n = node_tree.nodes.get(to_node)
            ...
            # Find the sockets
            from_s = None
            for sock in from_n.outputs:
                if sock.name == from_socket or sock.identifier == from_socket:
                    from_s = sock
                    break

            to_s = None
            for sock in to_n.inputs:
                if sock.name == to_socket or sock.identifier == to_socket:
                    to_s = sock
                    break
            ...
            # Create the link
            link = node_tree.links.new(from_s, to_s)

            return {
                "success": True,
                "message": f"Linked {from_node}.{from_socket} to {to_node}.{to_socket}",
            }
        except Exception as e:
            return {"error": str(e)}

    def set_node_property(self, material_name, node_name, property_name, value):
        ...
        try:
            ...
            # Try to set the property - could be an input or a node property
            # First try inputs
            if property_name in node.inputs:
                inp = node.inputs[property_name]
                # Handle different value types
                if hasattr(inp, "default_value"):
                    if isinstance(inp.default_value, (list, tuple)):
                        # Color or vector - expect a list
                        inp.default_value = value
                    else:
                        inp.default_value = value
                return {
                    "success": True,
                    "message": f"Set input '{property_name}' on node '{node_name}' to {value}",
                }

            # Then try node attributes
            if hasattr(node, property_name):
                setattr(node, property_name, value)
                return {
                    "success": True,
                    "message": f"Set property '{property_name}' on node '{node_name}' to {value}",
                }
    ...
    def assign_material(self, object_name, material_name, slot=None):
        ...
        try:
            ...
            if not hasattr(obj, "material_slots") and not hasattr(
                obj.data, "materials"
            ):
                return {
                    "error": f"Object '{object_name}' cannot have materials assigned"
                }

            # If slot is specified, use it
            if slot is not None:
                if slot < 0:
                    return {"error": "Slot index cannot be negative"}

                # Ensure enough material slots exist
                ...
                # No slot specified - add to end or assign to first empty slot
                if hasattr(obj.data, "materials"):
                    # Mesh object
                    obj.data.materials.append(mat)
                else:
                    # Other object types with material_slots
                    bpy.context.view_layer.objects.active = obj
                    obj.select_set(True)
                    bpy.ops.object.material_slot_add()
            ...
            return {
                "success": True,
                "message": f"Assigned material '{material_name}' to object '{object_name}'",
            }
        except Exception as e:
            return {"error": str(e)}
    ...
    def delete_material(self, material_name):
        ...
        try:
            ...
            return {"success": True, "message": f"Deleted material '{material_name}'"}
        except Exception as e:
            return {"error": str(e)}
    ...
    def set_keyframe(self, object_name, property_path, frame, value):
        ...
        try:
            ...
            # Handle array properties (e.g., "location[0]" for x location)
            if "[" in property_path and property_path.endswith("]"):
                base_path = property_path.split("[")[0]
                index = int(property_path.split("[")[1].rstrip("]"))

                # Get the property and set the value
                prop = getattr(obj, base_path, None)
                if prop is None:
                    return {"error": f"Property '{base_path}' not found on object"}

                prop[index] = value
                obj.keyframe_insert(data_path=f"{base_path}[{index}]")
            else:
                # Simple property
                if not hasattr(obj, property_path):
                    return {"error": f"Property '{property_path}' not found on object"}
                setattr(obj, property_path, value)
                obj.keyframe_insert(data_path=property_path)

            return {
                "success": True,
                "message": f"Keyframe set for '{property_path}' at frame {frame}",
                "object": object_name,
                "frame": frame,
                "value": value,
            }
    ...
    def delete_keyframe(self, object_name, property_path, frame):
        """Delete a keyframe for an object property at a specific frame"""
        try:
            obj = bpy.data.objects.get(object_name)
            if not obj:
                return {"error": f"Object '{object_name}' not found"}
            ...
            if "[" in property_path and property_path.endswith("]"):
                base_path = property_path.split("[")[0]
                index = int(property_path.split("[")[1].rstrip("]"))
                data_path = f"{base_path}[{index}]"
            else:
                data_path = property_path

            result = obj.keyframe_delete(data_path=data_path, frame=int(frame))

            if result:
                return {
                    "success": True,
                    "message": f"Keyframe deleted for '{property_path}' at frame {frame}",
                    "object": object_name,
                    "frame": frame,
                }
            else:
                return {
                    "success": False,
                    "message": f"No keyframe found for '{property_path}' at frame {frame}",
                    "object": object_name,
                    "frame": frame,
                }
        except Exception as e:
    ...
    def get_animation_info(self, object_name):
        ...
        try:
            ...
            return {"success": True, "animation_info": info}
        except Exception as e:
            return {"error": str(e)}

    def timeline_control(self, action, frame=None):
        ...
        try:
            ...
            if action == "play":
                ...
                return {
                    "success": True,
                    "action": "stop",
                    "message": "Animation stopped and reset to start",
                }
                ...
                if frame is not None:
                    ...
                    return {
                        ...
                        "action": "seek",
                        "frame": int(frame),
                        "message": f"Seeked to frame {frame}",
                    }
                else:
                    return {"error": "Frame parameter required for seek action"}

            else:
                return {
                    "error": f"Unknown action '{action}'. Must be: play, pause, stop, or seek"
                }
    ...
    def set_frame_range(self, start, end):
        """Set the scene frame range (start and end frames)"""
        try:
            scene = bpy.context.scene
            ...
            return {
                "success": True,
                "message": f"Frame range set to {start}-{end}",
    ...
    def get_fcurve_data(self, object_name, property_path=None):
        """Get fcurve keyframe data for an object, optionally filtered by property"""
        try:
            obj = bpy.data.objects.get(object_name)
            if not obj:
                return {"error": f"Object '{object_name}' not found"}
            ...
            if not obj.animation_data or not obj.animation_data.action:
                return {
                    "success": True,
                    "fcurves": [],
                    "message": f"Object '{object_name}' has no animation data",
                }
            ...
            for fcurve in action.fcurves:
                # Filter by property path if provided
                if property_path:
                    # Check if this fcurve matches the requested property
                    if "[" in property_path:
                        base_path = property_path.split("[")[0]
                        index = int(property_path.split("[")[1].rstrip("]"))
                        if fcurve.data_path != base_path or fcurve.array_index != index:
                            continue
                    elif fcurve.data_path != property_path:
                        continue
            ...
            return {
                ...
                "fcurves": fcurves_data,
            }
        except Exception as e:
            return {"error": str(e)}

    def get_nla_tracks(self, object_name):
        """Get NLA track information for an object"""
        try:
            obj = bpy.data.objects.get(object_name)
            if not obj:
                return {"error": f"Object '{object_name}' not found"}
    ...
    def create_nla_track(self, object_name, track_name):
        ...
        try:
            ...
            for existing_track in obj.animation_data.nla_tracks:
                if existing_track.name == track_name:
                    return {
                        "error": f"NLA track '{track_name}' already exists on object '{object_name}'"
                    }
            ...
            return {
                "success": True,
                "message": f"NLA track '{track_name}' created on object '{object_name}'",
    ...
    # ==================== End Animation Tool Handlers ====================

    # ==================== Visual Verification Tool Handlers ====================

    def capture_multi_view_screenshots(
        self, views="top,front,side,perspective", resolution=800, target_object=""
    ):
        ...
        try:
            ...
            # Camera positions for each view (location, rotation in degrees)
            VIEW_CONFIGS = {
            ...
            def capture_view(view_name, filepath):
                ...
                location, rotation = VIEW_CONFIGS[view_name]
                ...
                try:
                    ...
                    bpy.ops.render.opengl(
                        write_still=True, filepath=filepath, view_context=True
                    )
            ...
            for view in views_list:
                temp_path = os.path.join(
                    temp_dir, f"blender_multiview_{view}_{os.getpid()}.png"
                )
            ...
            return {
                "success": True,
                "views": result_views,
                "message": "Captured screenshots from multiple views",
            }
        except Exception as e:
            return {"error": str(e)}

    def export_screenshot_to_file(
        self, filepath="", view="perspective", resolution=1024, target_object=""
    ):
        ...
        try:
            ...
            if view not in VIEW_CONFIGS:
                return {"error": f"Unknown view type: {view}"}

            location, rotation = VIEW_CONFIGS[view]
            ...
            try:
                ...
                if os.path.exists(filepath):
                    return {
                        "success": True,
                        "filepath": filepath,
                        "message": "Screenshot exported successfully",
                    }
    ...
    def get_scene_analysis(self, collection_name=""):
        ...
        try:
            ...
            for obj in objects:
                ...
                if obj.type == "MESH" and obj.data:
                    ...
                    min_corner = Vector(
                        (min(v[i] for v in bbox_corners) for i in range(3))
                    )
                    max_corner = Vector(
                        (max(v[i] for v in bbox_corners) for i in range(3))
                    )
    ...
    def verify_geometry(
        self, object_name="", checks="manifold,normals,degenerate,overlapping,holes"
    ):
        ...
        try:
                ...
                try:
                    ...
                    if "normals" in checks:
                        ...
                        for face in bm.faces:
                            if face.normal.z < 0 and all(
                                v.co.z < face.calc_center_median().z for v in face.verts
                            ):
                    ...
                    if "degenerate" in checks:
                        ...
                        results["checks"]["degenerate"] = {
                            ...
                            "min_face_area": min(
                                (f.calc_area() for f in bm.faces), default=0
                            ),
                        }
            ...
            return {
                ...
                "message": "Geometry verification complete",
            }
        except Exception as e:
            return {"error": str(e)}

    # ==================== End Visual Verification Tool Handlers ====================
    ...
    def get_hunyuan3d_status(self):
        ...
        if enabled:
            match hunyuan3d_mode:
                case "OFFICIAL_API":
                    if (
                        not bpy.context.scene.blendermcp_hunyuan3d_secret_id
                        or not bpy.context.scene.blendermcp_hunyuan3d_secret_key
                    ):
                        return {
                            "enabled": False,
                            "mode": hunyuan3d_mode,
