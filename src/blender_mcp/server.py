...
from .responses import success_response, error_from_exception
...
@mcp.tool()
def create_material(ctx: Context, name: str, use_nodes: bool = True) -> str:
    """
    Create a new material in Blender.

    Parameters:
    - name: The name for the new material
    - use_nodes: Whether to enable node-based shading (default: True)

    Returns a JSON response with the material name and node status.
    """
    try:
        blender = get_blender_connection()
        result = blender.send_command(
            "create_material", {"name": name, "use_nodes": use_nodes}
        )

        if "error" in result:
            return json.dumps(error_from_exception(Exception(result["error"])))
...
@mcp.tool()
def delete_material(ctx: Context, material_name: str) -> str:
    """
    ...
    Returns a JSON response with success status and deleted material name.
    """
    try:
        blender = get_blender_connection()
        result = blender.send_command(
            "delete_material", {"material_name": material_name}
        )

        # Check for error response from Blender
            ...
            return json.dumps(
                {
                    "success": False,
                    "error": {
                        "code": error.get("code", "BLENDER_ERROR"),
...
def list_materials(ctx: Context) -> str:
    """
    List all materials in the Blender scene.

    Returns a list of materials with basic information including:
    - name: Material name
    - use_nodes: Whether the material uses shader nodes
    - users: Number of objects using this material
    """
    try:
        blender = get_blender_connection()
...
def set_keyframe(
    ...
    """
    Set a keyframe for an object property at a specific frame.

    Parameters:
    - object_name: The name of the object to keyframe
    - property_path: The property path to keyframe (e.g., "location", "rotation_euler", "scale", "location[0]")
    - frame: The frame number to insert the keyframe at
    - value: The value to set at this keyframe

    Returns a JSON response with the keyframe details.
    """
    try:
        blender = get_blender_connection()
        result = blender.send_command(
            ...
            return json.dumps(
                {
                    "success": False,
                    "error": {
                        "code": result["error"].get("code", "BLENDER_ERROR"),
...
def get_animation_info(ctx: Context, object_name: str) -> str:
    ...
    Returns a summary of animation data for the specified object including
    the list of animated properties, total keyframe count, and frame range.

    Parameters:
    - object_name: The name of the object to get animation info for

    Returns a JSON response with:
    - success: True/False
    - result: {"animated_properties": [...], "keyframe_count": int, "frame_range": [start, end] or None}
    """
    try:
        blender = get_blender_connection()
        result = blender.send_command(
...
@telemetry_tool("set_frame_range")
@mcp.tool()
def set_frame_range(ctx: Context, start: int, end: int) -> str:
    ...
    Parameters:
    - start: The start frame of the animation
    - end: The end frame of the animation

    Returns a JSON response with the frame range that was set.
    """
    try:
        blender = get_blender_connection()
        result = blender.send_command(
...
@telemetry_tool("get_fcurve_data")
@mcp.tool()
def get_fcurve_data(ctx: Context, object_name: str, property_path: str = None) -> str:
    """
    Get FCurve data for an object's animation.

    Returns the keyframe data for all FCurves (or filtered by property path)
    associated with the specified object's animation.

    Parameters:
    - object_name: The name of the object to get FCurve data for
    - property_path: Optional filter for a specific property (e.g., "location", "rotation_euler")

    Returns a JSON response with:
    - success: True/False
    - result: {"fcurves": [{"data_path": str, "keyframes": [{"frame": int, "value": float}]}]}
    """
    try:
        blender = get_blender_connection()
        result = blender.send_command(
        ...
        logger.error(f"Error getting fcurve data: {str(e)}")
        return json.dumps(error_from_exception(e))
...
@telemetry_tool("clear_cache")
@mcp.tool()
def clear_cache(ctx: Context) -> str:
    """Clear the MCP server cache.

    Removes all cached data from the server's memory cache.
    This does not affect any Blender data or persistent storage.
    """
    from .cache import cache

    cleared_count = cache.clear()
    return json.dumps(
        {"success": True, "result": {"cleared": True, "entries_removed": cleared_count}}
    )


@telemetry_tool("cache_info")
@mcp.tool()
def cache_info(ctx: Context) -> str:
    ...
    return json.dumps({"success": True, "result": cache.cache_info()})


# Shader Node Tools

# Mapping of user-friendly node type names to Blender internal names
NODE_TYPE_MAP = {
...
@telemetry_tool("create_shader_node")
@mcp.tool()
def create_shader_node(
    ...
    """
    Create a shader node in a material's node tree.

    Parameters:
    - material_name: Name of the material to add the node to
    - node_type: Type of node to create (BSDF_PRINCIPLED, TEX_IMAGE, TEX_COORD,
                MAPPING, MIX, NORMAL_MAP, OUTPUT_MATERIAL, MATH, RGB, VALUE)
    - name: Optional custom name for the node
    - location: Optional [x, y] position for the node (default: [0, 0])

    Returns a JSON response with success status and node details.
    """
    # Default location
    if location is None:
        location = [0, 0]
        ...
        return json.dumps(
            {
                ...
                "error": {
                    "code": "INVALID_INPUT",
                    "message": f"Invalid node_type '{node_type}'. Must be one of: {', '.join(NODE_TYPE_MAP.keys())}",
                },
            }
        )
    ...
    try:
        ...
        logger.error(f"Error creating shader node: {str(e)}")
        return json.dumps(error_from_exception(e))


@telemetry_tool("link_shader_nodes")
@mcp.tool()
def link_shader_nodes(
    ctx: Context,
    material_name: str,
    from_node: str,
    from_socket: str,
    ...
    """
    Link two shader nodes together by connecting their sockets.
    ...
    Returns a JSON response with success status and linked result.
    """
    try:
        blender = get_blender_connection()
        result = blender.send_command(
        ...
        logger.error(f"Error linking shader nodes: {str(e)}")
        return json.dumps(error_from_exception(e))
...
def assign_material(
    ...
    """
    Assign a material to an object at a specific slot index.
    ...
    Returns a JSON response with success status and assignment details.
    """
    try:
        blender = get_blender_connection()
        result = blender.send_command(
...
@telemetry_tool("set_node_property")
@mcp.tool()
def set_node_property(
    ...
    """
    Set a property value on a shader node.

    Common properties for Principled BSDF nodes:
    - Base Color: RGBA color as [r, g, b, a] (0.0-1.0)
    - Metallic: Float (0.0-1.0)
    - Roughness: Float (0.0-1.0)
    - IOR: Float (1.0+ for glass-like materials)
    - Normal: Normal vector

    Parameters:
    - material_name: Name of the material containing the node
    - node_name: Name of the shader node (e.g., "Principled BSDF")
    - property: Name of the property/input to set (e.g., "Base Color", "Metallic")
    - value: The value to set (float for scalar, list for color/vector)

    Returns a JSON response with success status and the updated property.
    """
    try:
        blender = get_blender_connection()
        result = blender.send_command(
        ...
        logger.error(f"Error setting node property: {str(e)}")
        return json.dumps(error_from_exception(e))


@telemetry_tool("get_node_tree")
@mcp.tool()
def get_node_tree(ctx: Context, material_name: str) -> str:
    """
    Get the shader node tree for a material.

    Returns a structured representation of all nodes and their connections.
    This provides a summary of the node tree structure including node names,
    types, locations, and key properties, as well as the links between nodes.

    Parameters:
    - material_name: Name of the material to get the node tree from

    Returns a JSON response with:
    - success: True/False
    - result: {"nodes": [...], "links": [...]}
      - Each node: {name, type, location, properties}
      - Each link: {from_node, from_socket, to_node, to_socket}
    """
    try:
        blender = get_blender_connection()
        result = blender.send_command(
        ...
        logger.error(f"Error getting node tree: {str(e)}")
        return json.dumps(error_from_exception(e))


@telemetry_tool("timeline_control")
@mcp.tool()
def timeline_control(
    ...
    """
    Control the animation timeline playback.

    Supported actions:
    - play: Start or resume animation playback
    - pause: Pause animation playback (maintains current frame)
    - stop: Stop animation playback (returns to start frame)
    - seek: Jump to a specific frame (requires frame parameter)

    Parameters:
    - action: The timeline action to perform (play, pause, stop, seek)
    - frame: The frame number to seek to (required for seek action, ignored otherwise)

    Returns a JSON response with:
    - success: True/False
    - result: {"action": str, "current_frame": int, "is_playing": bool}
    """
    # Validate action
    ...
    # Validate seek action has frame parameter
    if action == "seek" and frame is None:
        return json.dumps(
            {
                "success": False,
                "error": {
                    "code": "INVALID_INPUT",
                    "message": "Frame parameter is required for seek action",
                },
            }
        )
    ...
    try:
        ...
        logger.error(f"Error controlling timeline: {str(e)}")
        return json.dumps(error_from_exception(e))


@telemetry_tool("create_nla_track")
@mcp.tool()
def create_nla_track(
    ...
    """
    Create a new NLA track for an object.

    NLA (Non-Linear Animation) tracks allow organizing and blending multiple
    animation actions. The track will be created on the object's animation data.

    Parameters:
    - object_name: Name of the object to create the NLA track on
    - track_name: Name for the new NLA track

    Returns a JSON response with:
    - success: True/False
    - result: {"name": str} - The name of the created track
    """
    try:
        blender = get_blender_connection()
        result = blender.send_command(
...
@telemetry_tool("delete_keyframe")
@mcp.tool()
def delete_keyframe(
    ctx: Context,
    object_name: str,
    ...
    """
    Delete a keyframe from an object's property at a specific frame.

    Parameters:
    - object_name: Name of the object to delete the keyframe from
    - property_path: The property path (e.g., "location", "rotation_euler", "scale")
    - frame: The frame number to delete the keyframe from
...
@telemetry_tool("get_nla_tracks")
@mcp.tool()
def get_nla_tracks(ctx: Context, object_name: str) -> str:
    """
    Get the NLA tracks for an object.

    Returns a list of NLA tracks with their strips (name and frame range only).

    Parameters:
    - object_name: Name of the object to get NLA tracks from

    Returns a JSON response with:
    - success: True/False
    - result: {"tracks": [{"name": str, "strips": [...]}]}
      - Each strip: {name, frame_start, frame_end}
    """
    try:
        blender = get_blender_connection()
        result = blender.send_command(
        ...
        logger.error(f"Error getting NLA tracks: {str(e)}")
        return json.dumps(error_from_exception(e))
